...
* telemetry, motor control with PID-based heading correction, IR aremote
...
/* Smoke sensor */
const int SMOKE_PIN = 4; //mcp // Smoke sensor (MCP23017 pin)
/* Buzzer */
const int BUZZER_PIN = 5; //mcp // Buzzer (MCP23017 pin)
/* Fire Sensor */
...
if (!manualMode){
  ...
  if (dtLine >= 0.01) { 
    // Αντί για Derivative που "κλωτσάει" το πρόσημο, 
    // χρησιμοποιούμε έναν συντελεστή εξομάλυνσης (α).
    // Όσο μικρότερο το alpha, τόσο μεγαλύτερη η "αντίσταση".
    
    float alpha = 0.2; // Τιμή από 0.1 έως 1.0 (0.2 = 80% αντίσταση)
    static float filteredTargetAngle = 0;

    // Αυτή η εξίσωση κάνει το targetAngle να "γλιστράει" προς τη νέα τιμή 
    // αντί να πηδάει ακαριαία, εξαλείφοντας το sine wave.
    filteredTargetAngle = (alpha * targetAngle) + ((1.0 - alpha) * filteredTargetAngle);

    // Κλήση με τη φιλτραρισμένη γωνία
